...
static size_t manager_get_remaining_capacity(queue_manager_t* manager, uint64_t key) {
	...
	size_t remaining = queue == NULL ? (int32_t) manager->queue_buffer_capacity : queue->buffer_capacity - queue->buffer_size;
...
CS_EXPORT queue_manager_t* CS_CALL Create(size_t queue_buffer_capacity, int64_t packet_interval) {
	return manager_create(queue_buffer_capacity, packet_interval);
}

CS_EXPORT void CS_CALL Destroy(queue_manager_t* instance) {
	manager_destroy(instance);
}

CS_EXPORT size_t CS_CALL GetRemainingCapacity(queue_manager_t* instance, uint64_t key) {
	return manager_get_remaining_capacity(instance, key);
}

// address_string is null-terminated (TODO releasing string?)
CS_EXPORT booltype CS_CALL QueuePacket(queue_manager_t* instance, uint64_t key, const char* address_string, int32_t port, void* data_buffer, size_t data_length) {
	return manager_queue_packet(instance, key, address_string, port, data_buffer, data_length);
}

CS_EXPORT void CS_CALL Process(queue_manager_t* instance) {
